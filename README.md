...
4. **Attach to a live libp2p host**: `wireLibp2pDCUtRMetrics(libp2p)` bridges native `hole-punch:*`, `relay:*`, and `stream:migrate` events plus nested `libp2p.services.dcutr` buses into the same Prometheus surfaces with deduped attempt tracking so real punch flows and synthetic harness traffic co-exist without double-counting.【F:src/observability/dcutrEvents.ts†L1-L211】【F:test/observability/dcutrEvents.test.ts†L1-L131】
