  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      if (
        ...
        return send(403, { error: 'Invalid host or origin' });
      const path = new URL(req.url, origin).pathname;
      const assets = {
        '/': ['operator.html', 'text/html'],
        '/operator.js': ['operator-ui.js', 'text/javascript'],
        '/operator.css': ['operator.css', 'text/css'],
      };
      if (req.method === 'GET' && assets[path]) {
        res.writeHead(200, { 'Content-Type': assets[path][1] });
        return res.end(
          await readFile(new URL(`./web/${assets[path][0]}`, import.meta.url)),
        );
      }
      ...
      if (
        ...
        return send(401, { error: 'Operator token required' });
      if (req.method === 'GET' && path === '/api/status') {
        ...
        return send(200, {
          ...
          head: n.head,
          ledgerVerified: true,
          operations: operationStatus(n),
          outcomes: outcomeSummary(n),
          runtime: [...n.runtime.values()]
            .slice(-100)
            .reverse()
            .map((e) => ({
              id: e.id,
              topic: e.topic,
              at: e.at,
              status: e.data.status ?? e.data.plan?.status ?? 'recorded',
      ...
      if (req.method === 'GET' && path === '/api/evidence') {
        const n = await loadNode(dir);
        const run = n.runs.get(new URL(req.url, origin).searchParams.get('id'));
      ...
      if (req.method === 'GET' && path === '/api/qualification')
        return send(200, await qualifyNode(dir));
      if (req.method !== 'POST')
      ...
      if (path === '/api/review')
        return send(200, await importDetachedReview(dir, body));
