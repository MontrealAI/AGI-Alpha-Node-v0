...
import Ajv from 'ajv';
import draft7MetaSchema from 'ajv/lib/refs/json-schema-draft-07.json' assert { type: 'json' };
...
describe('alpha-wu schema validation', () => {
  it('validates sample payload with JSON schema and zod schema', () => {
    ...
    const ajv = new Ajv({ strict: false, allErrors: true, schemaId: 'auto' });
    // Ensure backward compatibility with the draft-07 meta schema bundled with Ajv v6
    if (!ajv.getSchema('http://json-schema.org/draft-07/schema')) {
      ajv.addMetaSchema(draft7MetaSchema);
    }
    const draft2020Compat = JSON.parse(JSON.stringify(draft7MetaSchema));
    draft2020Compat.$id = 'https://json-schema.org/draft/2020-12/schema';
    if (!ajv.getSchema(draft2020Compat.$id)) {
      ajv.addMetaSchema(draft2020Compat);
    }
