...
- **Hard ceilings everywhere**: `buildResourceManagerConfig` enforces global caps (connections, streams, memory bytes, FDs, bandwidth) with a tunable `NRM_SCALE_FACTOR` (0.5–2.0) plus optional JSON/YAML overrides via `NRM_LIMITS_JSON` or `NRM_LIMITS_PATH` for per-protocol/per-peer budgets.【F:src/network/resourceManagerConfig.js†L34-L119】
...
| --- | --- | --- | --- |
| Global limits | `NRM_SCALE_FACTOR`, `NRM_LIMITS_JSON`, `NRM_LIMITS_PATH` | `1` + built-ins | Caps conns/streams/memory/FDs/bandwidth; inline or file overrides for protocol/peer edges.【F:src/network/resourceManagerConfig.js†L34-L119】 |
