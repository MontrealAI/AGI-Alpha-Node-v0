...
for (const name of ['cycle', 'pause', 'resume']) $(name).onclick = () => action(async () => { $(name).disabled = true; try { const result = await api(`/api/${name}`, {}); await refresh(); if (result.status) notify(`Cycle: ${result.status}${result.missionId ? ` · ${result.missionId}` : ''}`); } finally { $(name).disabled = false; } });
