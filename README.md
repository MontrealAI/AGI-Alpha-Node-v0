...
- **Registry‑backed thresholding + replay ledger:** The orchestrator tallies guardian envelopes against a registry, rejects duplicates or unknown keys, and flips the execution switch only when the M‑of‑N quorum is satisfied for the exact digest under review—then stamps the digest + tx hash into `IntentLedger` so the same intent cannot be reused.【F:src/treasury/guardianRegistry.ts†L1-L54】【F:src/treasury/thresholdAggregator.ts†L1-L56】【F:src/treasury/intentLedger.ts†L1-L83】
