...
3. Expose `/metrics` through your existing Prometheus HTTP handler (or use `startMonitoringServer({ dcutrEventSource, dcutrEventEmitter })` to auto-register the DCUtR counters, gauges, and histograms on the Alpha Node telemetry endpoint).【F:src/telemetry/monitoring.js†L273-L332】
