...
| --- | --- | --- | --- |
| Contracts | [`contracts/AlphaNodeManager.sol`](contracts/AlphaNodeManager.sol), [`contracts/TreasuryExecutor.sol`](contracts/TreasuryExecutor.sol) | Canonical `$AGIALPHA` control plane anchored at `0xa61a3b3a130a9c20768eebf97e21515a6046a1fa` with pause, validator, identity, staking, and treasury levers. | Pause, gate validators, rotate ENS controllers, sweep stake, and re-route flows without redeploying. |
| Runtime | [`src/index.js`](src/index.js), [`src/network`](src/network), [`src/orchestrator`](src/orchestrator), [`src/intelligence`](src/intelligence) | Libp2p host, resource manager, intelligence plane, and orchestration interfaces. | Adjust runtime posture while observing metrics and events. |
| Telemetry | [`observability/prometheus/alerts.yml`](observability/prometheus/alerts.yml), [`grafana/provisioning/dashboards`](grafana/provisioning/dashboards) | Prometheus, Alertmanager, and Grafana dashboards pre-provisioned for first-boot visibility. | Confirm pauses, validator gates, and ENS rotations via dashboards and alerts. |
| Dashboard | [`dashboard/`](dashboard/) | Vite/React cockpit for operators. | Review runtime posture and validator state with the same signals emitted in CI. |
| Subgraph | [`subgraph/`](subgraph/) | Indexes contract events for dashboards and auditability. | Keep validator, staking, and identity data coherent with on-chain authority. |
| Scripts | [`scripts/`](scripts/) | ENS inspection, treasury intents, P2P simulation, Grafana lint, branch/health gates. | Execute runbooks quickly and mirror the CI wall locally. |
...
- **Required checks:** [`.github/required-checks.json`](.github/required-checks.json) lists the exact status checks to enforce in GitHub branch protection (apply to `main` and all PRs).
- **Local parity:** `npm run ci:verify` executes the same gates so contributors can keep the badge wall green before opening a PR.
- **Branch rule recipe:** Settings → Branches → Add rule for `main` → Require a pull request before merging → Require status checks to pass using [`.github/required-checks.json`](.github/required-checks.json) → Require conversation resolution and “Require branches to be up to date.”
